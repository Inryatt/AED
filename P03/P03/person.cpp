...
class person
{
  private:
    ...
    int p_number;
  public:
    person(const char *name = "",const char *address = "", const int pnumber = 000000000); // constructor
    ~person(void);                                          // destructor
    void set_name(const char *name);
    void set_address(const char *address);
    void set_pnumber(const int pnumber);
...
void person::set_pnumber(const int pnumber)
{
  p_number=pnumber;
}

person::person(const char *name,const char *address, const int pnumber)
{ // constructor
  if(debug != 0)
    cout << "person constructor called" << endl;
  this->set_name(name);
  this->set_address(address);
  this->set_pnumber(pnumber);
}
...
int main(void)
{
  person p = person("John Doe","no address",919240024); // create a new object and then copy it to p
  person q("no name","no address",917006729);           // create a new object (no copy needed)
